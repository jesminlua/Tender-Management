...
async def get_page_html(page, site):
    """Navigate and wait properly before extracting HTML."""
    wait_sel = site.get("wait_for_selector")
    delay = site.get("delay_ms", 2000) / 1000

    # Wait for selector if configured
    if wait_sel:
        try:
            await page.wait_for_selector(wait_sel, timeout=20000)
            log.info(f"  wait_for_selector '{wait_sel}' found.")
        except Exception:
            log.warning(f"  wait_for_selector '{wait_sel}' timed out — continuing anyway.")

    # Always wait the configured delay
...
async def process_job(job, supabase):
    ...
    site = res.data
    site_url = site["url"]
    log.info(f"  Scraping site: {site['name']} — {site_url}")
    ...
    try:
        ...
        async with acquire_tab(cookies=saved_cookies) as (context, page):
            ...
            async for page_num in pag_mod.paginate(page, site):
                ...
                html = await get_page_html(page, site)
