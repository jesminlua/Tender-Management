...
- IGNORE any items from "Quotation Result", "Tender Result", "Quotation Cancellation", "Tender Cancellation" sections
- SKIP any item that has a "Successful Bidders" field - those are historical results not active tenders
- Only extract ACTIVE open tenders/quotations that are currently accepting submissions
...
If zero active tenders found, return [].
...
Return a JSON object with these keys (null if not found):
  ...
  documents         - list of document names available for download
...
Return a JSON object with these keys (null if not found):
  ...
  submission_method - how and where to submit
  contact_name      - contact person name
...
def merge_detail_into_tender(tender, detail):
    ...
    parts = list(filter(None, [detail.get("contact_name"), detail.get("contact_email"), detail.get("contact_phone")]))
    if parts:
